...
def button1_click():
    ...
    while True:
        ...
        cv2.imshow("Hand volume control using python done by venkata reddy",image)
        key=cv2.waitKey(10)
        if key==27:
            break
    webcam.release()
...
window=tk.Tk()
window.title("Button Example")
button1 = tk.Button(window, text="Volume Control", command=button1_click)
button1.pack()
...
button4 = tk.Button(window, text="Take Screenshot", command=button4_click)
button4.pack()
